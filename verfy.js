...
async function main() {
    ...
    for (var val in addres) {
        let paramfiletoLower = addres[val].contractFile.toLowerCase()
        let paramfile = paramfiletoLower
        //console.log(paramfile)
        param = filejson[paramfile]
        if(param==undefined){
            console.log(`< ${val} > contract file not exist `)
        }else {
            command = command + `npx hardhat --network ${network[0]} verify ${addres[val].address} --contract ${param}:${addres[val].contractName} --constructor-args  ./param/${val}.js\n`
        }
